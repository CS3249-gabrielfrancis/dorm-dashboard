...
Meteor.startup(() => {
    // Query if Heroku deployment
    if (process.env.IS_HEROKU == 'true') {
        // Check if datacollection is present.
        if (!DatasCollection.find().fetch() > 0) {
            ...
            // Empty data is removed on heroku - admin to manually purge if data is dirty
            // Static data used, so this is ok for now.
